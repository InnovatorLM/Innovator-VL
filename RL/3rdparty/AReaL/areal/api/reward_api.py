...
class AsyncRewardWrapper:
    ...
    async def __call__(self, *args, **kwargs) -> float:
        ...
        for attempt in range(self.max_retries + 1):
            ...
            if executor is None:
                # Try to recreate executor if it was removed
                if attempt < self.max_retries:
                    logger.warning("ProcessPoolExecutor was removed. Attempting to recreate...")
            ...
            try:
                ...
                last_exception = e
                # Check if executor was shut down - need to recreate it
                is_shutdown_error = (
                ...
                if is_shutdown_error:
                    logger.warning(
                        f"ProcessPoolExecutor was shut down (attempt {attempt + 1}/{self.max_retries + 1}). "
                        "Attempting to recreate..."
                    )
                    if attempt < self.max_retries:
                        # Try to recreate the executor
                        ...
                        logger.error("Max retries exceeded for executor shutdown error.")
                        traceback.print_exc()
                        raise e
                else:
