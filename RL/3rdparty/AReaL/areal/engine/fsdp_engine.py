...
class FSDPEngine(BaseHFEngine):
    ...
    def _load_model_from_hf(self, path: str):
        ...
        if dist.get_rank() == 0:
            ...
            # Convert parameter names to match model structure
            # For LLaVA-OneVision models: convert model.* to model.language_model.* and visual.* to model.visual.*
            converted_state = {}
            for name, weight in full_state.items():
                if name.startswith("model.layers") or name.startswith("model.embed_tokens") or name.startswith("model.norm"):
                    # Convert model.* to model.language_model.*
                    new_name = name.replace("model.", "model.language_model.", 1)
                    converted_state[new_name] = weight
                elif name.startswith("visual."):
                    # Convert visual.* to model.visual.*
                    new_name = "model." + name
                    converted_state[new_name] = weight
                else:
                    # Keep other parameters as is (e.g., lm_head.*)
                    converted_state[name] = weight
            full_state = converted_state
        else:
            full_state = {}
