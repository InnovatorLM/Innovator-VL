...
def local_main(config, run_id: int = 0):
    ...
    if alloc_mode.type_ != AllocationType.LLM_SERVER_ONLY:
        ...
        launcher.submit(
            job_name="trainer",
            ### zichen add
            # cmd=f"torchrun --nnodes 1 --nproc-per-node {nprocs} --master-addr localhost --master-port {find_free_ports(1, 
            # (10000, 50000))[0]} {' '.join(sys.argv[1:])}",
            cmd=f"python3 -m torch.distributed.run --nnodes 1 --nproc-per-node {nprocs} --master-addr localhost --master-port {find_free_ports(1, (10000, 50000))[0]} {' '.join(sys.argv[1:])}",
            ### zichen add
            gpu=gpu,
            env_vars=dict(
                **get_env_vars(
