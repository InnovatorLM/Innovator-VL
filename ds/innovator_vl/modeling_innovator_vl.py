...
from innovator_vl.configuration_innovator_vl import InnovatorVlConfig, InnovatorVl_TextConfig, RiceConfig
...
@dataclass
class InnovatorVl_ModelOutputWithPast(ModelOutput):
    """
    Base class for Innovator-VL outputs, with hidden states and attentions.
...
@dataclass
class InnovatorVl_CausalLMOutputWithPast(ModelOutput):
    """
    Base class for Innovator-VL causal language model (or autoregressive) outputs.
...
class InnovatorVl_RotaryEmbedding(nn.Module):
    def __init__(self, config: InnovatorVl_TextConfig, device=None):
        super().__init__()
...
class InnovatorVl_Attention(nn.Module):
    ...
    def __init__(self, config: InnovatorVl_TextConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class InnovatorVl_FlashAttention2(InnovatorVl_Attention):
    """
    InnovatorVl flash attention module, following Qwen2VL attention module. This module inherits from `InnovatorVl_Attention`
...
class InnovatorVl_SdpaAttention(InnovatorVl_Attention):
    """
    InnovatorVl1.5 attention module using torch.nn.functional.scaled_dot_product_attention. This module inherits from
...
class InnovatorVl_DecoderLayer(nn.Module):
    def __init__(self, config: InnovatorVl_TextConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class InnovatorVl_TextModel(Qwen2VLPreTrainedModel):
    config_class = InnovatorVl_TextConfig

    def __init__(self, config: InnovatorVl_TextConfig):
        super().__init__(config)
            ...
            if attention_mask is not None and past_key_values is not None:
                ...
                if is_padding_right:
                    raise ValueError(
                        ...
                        " this may lead to unexpected behaviour for Flash Attention version of Innovator-VL. Make sure to "
...
@auto_docstring
class InnovatorVl_Model(Qwen2VLPreTrainedModel):
    base_model_prefix = ""
    ...
    def __init__(self, config: InnovatorVlConfig):
        ...
        self.language_model = InnovatorVl_TextModel._from_config(config.text_config)
    ...
    def forward(
        ...
        output = InnovatorVl_ModelOutputWithPast(
...
class InnovatorVl_ForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = InnovatorVl_Model(config)
    ...
    def forward(
        ...
        >>> model = AutoModelForCausalLM.from_pretrained("Deep-VLM/Innovator-VL-4b", trust_remote_code=True)
        >>> processor = AutoProcessor.from_pretrained("Deep-VLM/Innovator-VL-4b", trust_remote_code=True)
        ...
        return InnovatorVl_CausalLMOutputWithPast(
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states,
...
__all__ = ["InnovatorVl_ForConditionalGeneration", "InnovatorVl_Model", "Qwen2VLPreTrainedModel", "InnovatorVl_TextModel"]


AutoConfig.register("innovator_vl", InnovatorVlConfig)
AutoModelForCausalLM.register(InnovatorVlConfig, InnovatorVl_ForConditionalGeneration)