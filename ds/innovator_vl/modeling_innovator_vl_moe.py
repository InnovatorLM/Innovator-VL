...
from transformers.modeling_outputs import BaseModelOutputWithPast, ModelOutput, MoeModelOutputWithPast, MoeCausalLMOutputWithPast
...
from innovator_vl.configuration_qwen3_vl_moe import InnovatorVlConfig, InnovatorVl_TextConfig, RiceConfig
...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        cu_seqlens = torch.repeat_interleave(grid_thw[:, 1] * grid_thw[:, 2]+1, grid_thw[:, 0]).cumsum(
            dim=0,
        ...
        return self.merger(hidden_states) #FIXME


    def forward_debug(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> dict:
        r"""
        grid_thw (`torch.LongTensor` of shape `(num_images, 3)`):
            The temporal, height and width dimensions of feature shape for each image. Each row contains [t, h, w] values.
        """
        ...
        rotary_pos_emb = self.rot_pos_emb(grid_thw)
        output["rotary_pos_emb_original"] = rotary_pos_emb.clone()
        
        # Add class position embedding
        rotary_pos_emb = torch.cat([self.class_pos_emb, rotary_pos_emb], dim=0)
        output["rotary_pos_emb_with_class"] = rotary_pos_emb.clone()
        
        # Add class embedding to hidden states
        hidden_states = torch.cat(
            [self.class_embedding.to(hidden_states.dtype).unsqueeze(0),
            hidden_states], dim=0
        )
        output["after_class_embedding"] = hidden_states.clone()
        ...
        emb = torch.cat((rotary_pos_emb, rotary_pos_emb), dim=-1)
        position_embeddings = (emb.cos(), emb.sin())
        output["position_embeddings_cos"] = position_embeddings[0].clone()
        output["position_embeddings_sin"] = position_embeddings[1].clone()
        ...
        # Store input to transformer blocks
        output["before_transformer_blocks"] = hidden_states.clone()
        
        # Pass through transformer blocks
        for i, blk in enumerate(self.blocks):
            hidden_states_before_block = hidden_states.clone()
            ...
            # Store outputs for first few blocks to avoid memory issues
            if i < 3:  # Only store first 3 blocks
                output[f"after_block_{i}"] = hidden_states.clone()
                output[f"before_block_{i}"] = hidden_states_before_block
        
        output["after_all_blocks"] = hidden_states.clone()
        ...
        # Pass through merger
        merged_output = self.merger(hidden_states)
        output["after_merger"] = merged_output.clone()
        
        # Store grid information for debugging
        output["grid_thw"] = grid_thw.clone()
        output["sequence_length"] = torch.tensor(hidden_states.shape[0])
        output["hidden_dim"] = torch.tensor(hidden_states.shape[1])
