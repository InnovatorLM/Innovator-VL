from innovator_vl.configuration_innovator_vl import InnovatorVlConfig
...
def load_vit_weights(model, vit_path):
    """
    Load ViT weights and copy them to the vision part of InnovatorVl_ForConditionalGeneration
    
    Args:
        model: InnovatorVl_ForConditionalGeneration
...
def load_adapter_weights(model, adapter_path, cur_len):
    """
    Load Adapter weights and copy them to the corresponding part of InnovatorVl_ForConditionalGeneration
    
    Args:
        model: InnovatorVl_ForConditionalGeneration model
...
def validate_vit_consistency(model, vit_path, img_path):
    ...
    """
    print(f"Verifying consistency of ViT component using image: {img_path}")
    
    # 修改这里：支持本地文件路径
    if os.path.exists(img_path):
        sample_image = Image.open(img_path).convert("RGB")
    else:
        # 如果不是本地文件，再尝试当做 URL 处理（虽然离线环境用不到，但保留逻辑）
        try:
...
def validate_llm_consistency(model, llm_path, sample_text):
    ...
    Args:
        model: Merged InnovatorVl_ForConditionalGeneration model
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path", type=str, default="./checkpoints/merged/Innovator-VL-4B-stage0", help="Path to save the merged model")
