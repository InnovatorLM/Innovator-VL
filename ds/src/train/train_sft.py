...
from innovator_vl.modeling_innovator_vl.py import InnovatorVl_ForConditionalGeneration
...
def train():
    ...
    try:
        if "Qwen2.5" in model_args.model_id:
            ...
            rank0_print("Loading Innovator-VL model...")
            model = InnovatorVl_ForConditionalGeneration.from_pretrained(
                model_args.model_id,
                torch_dtype=compute_dtype,
                attn_implementation="flash_attention_2" if not training_args.disable_flash_attn2 else "sdpa", 
                **bnb_model_from_pretrained_args
