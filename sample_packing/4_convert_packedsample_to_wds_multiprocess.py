...
import sys
sys.path.append('/root/innovator_code_wenzichen/Innovator-VL/aiak_megatron')
sys.path.append('/root/innovator_code_wenzichen/Innovator-VL/aiak_megatron/megatron')
...
from tool import get_init_file,cfg
...
# 生成 1 条数据
def stream_samples_caption(json_paths: str):
...
def construct_sample_caption(args, entry):
    ...
    for idx, img_name in enumerate(entry["images"]):
        img_path = img_name
        # os.path.join(args.image_dir,f'{entry["id"]}.{img_name}')
        if cfg['image'].get('effective_in_wds', False):
            ...
            buf = io.BytesIO()
            img_input.save(buf, format="JPEG",quality=70, optimize=True)   # 无损压缩，体积大，质量高
            sample[f"img{idx}.jpg"]=buf.getvalue()
...
def convert_to_wds(chunk_jsonPaths,args):
    """ Convert dataset to wds format """
    
    pid = os.getpid()
    print(args)
    tar = os.path.join(args.output_dir, f'pretrain{pid}-%06d.tar')
    if args.mode == "caption_pack":
        # 新模式
        with wds.ShardWriter(tar, maxcount=args.maxcount, maxsize=args.maxsize) as sink:
            for entry in tqdm(stream_samples_caption(chunk_jsonPaths)):
                try:
...
def mutiprocess_main(num_processes):
    args = parse_args()
    if not os.path.exists(args.output_dir):
        os.mkdir(args.output_dir)
    jsonps=list(Path(args.json_file).glob("*.json"))
    print(len(jsonps))
    chunk_size = max(1, len(jsonps) // num_processes) 
    chunks_jsonPaths = [jsonps[i:i+chunk_size] for i in range(0, len(jsonps), chunk_size)]
    convert_func = partial(convert_to_wds, args=args)
    with Pool(processes=num_processes) as pool:
        list(pool.imap_unordered(convert_func, chunks_jsonPaths))
...
if __name__ == '__main__':
    mutiprocess_main(num_processes=20)
